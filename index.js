(() => {
    ...
    function jsonToMermaid(json) {
        ...
        json.nodes.forEach((n) => {
            const icons = (n.image ? "📷 " : "") + (n.link ? "🔗 " : "");
            const label = icons + n.text;
            const shape = label.includes("?") ? `{${label}}` : `[${label}]`;
            code += `  ${n.id}${shape}\n`;
        });
        ...
        try {
            ...
            setTimeout(() => {
                ...
                labels.forEach((label) => {
                    ...
                    label.onclick = () => {
                        const nodeText = label.textContent
                            .replace("📷", "")
                            .replace("🔗", "")
                            .trim();
                        const json = JSON.parse(editor.getValue());
                        const matchedNode = json.nodes.find(
                            (n) => n.text.trim() === nodeText
                        );

                        if (!matchedNode) return;

                        if (matchedNode.link) {
                            // alert('link found' + matchedNode.link)
                            window.open(matchedNode.link, "_blank");
                            return;
                        }
